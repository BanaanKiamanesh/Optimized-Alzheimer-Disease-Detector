        ...
        parfor i = 1:HabitationNum
            for k = 1:VarNum
                ...
                if rand <= Lambda(i)
                    ...
                    % Migration
                    newhbt(i).SIV(k) = ceil(hbt(i).SIV(k) ...
                        +Alpha*(hbt(j).SIV(k)-hbt(i).SIV(k)));

                    if newhbt(i).SIV(k)>Range(2)
                        newhbt(i).SIV(k) = Range(2);
                    end
                end
...
function j = RouletteWheelSelection(P)
    ...
    % Function for Probablistic RouletterWheel Selection
